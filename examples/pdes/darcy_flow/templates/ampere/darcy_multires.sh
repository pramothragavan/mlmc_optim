#!/bin/bash
#SBATCH --job-name=darcy-multires-ampere
...
OUT_DIR="${OUT_DIR:-results/darcy_ampere_multires_seed42}"
...
"${PYTHON}" examples/pdes/darcy_flow/runners/run_experiment.py \
  ...
  --epochs_per_phase_json '[100, 100, 100, 100, 100]' \
  --c2f_res_per_phase_json '[[15], [30], [60], [120], [241]]' \
  --subset_size_per_phase_json '[[1024], [1024], [1024], [1024], [1024]]' \
  --levels_per_phase_json '[1, 1, 1, 1, 1]' \
  --batch_size_per_phase_json '[[160], [80], [80], [20], [20]]' \
  "${coord_args[@]}" \
